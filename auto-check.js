(() => {
  ...
  const answerKey = Array.isArray(window.HOUHOU_ANSWER_KEY)
  ...
  function loadState() {
    try {
      ...
      return {
        answers: cards.map((_, index) =>
          typeof parsed.answers[index] === "string" ? parsed.answers[index] : ""
        ),
        judgments: cards.map((_, index) =>
          parsed.judgments[index] === "correct" ? "correct" : null
        )
      };
    } catch {
      return emptyState();
  ...
  function acceptedAnswers(index) {
    const configured = answerKey[index];
  ...
  function isCorrect(index, value) {
    ...
    return normalized && acceptedAnswers(index).includes(normalized);
  }
  ...
  style.textContent = `
    ...
    .save-answer:hover:not(:disabled) {
  ...
  function updateProgress() {
    ...
    if (questionCounter) {
      questionCounter.textContent = `回答正确 ${correctCount} / ${cards.length}`;
    }
    ...
    document.querySelectorAll(".milestone-card").forEach((card) => {
      ...
      if (milestoneState) {
        milestoneState.textContent = unlocked
          ? "✓ 已解锁"
          : `还差 ${remaining} 个正确回答`;
      }
    });
  ...
  function enhanceCard(card, index) {
    ...
    button.addEventListener("click", () => {
      ...
      if (!acceptedAnswers(index).length) {
        window.alert("这一题还没有设置正确答案，请联系出题人。 ");
        return;
      }
  ...
  function updatePageCopy() {
    ...
    if (description) {
      description.textContent = "自由选择问题并输入答案。系统会自动判断，答错后可以继续修改。";
    }
    ...
    if (list) {
      list.innerHTML = `
        ...
        <li><span class="rule-number">2</span><span>在回答框中填写答案，然后点击 <strong>“提交答案”</strong>。</span></li>
        <li><span class="rule-number">3</span><span>答案正确会自动计入通关进度；答案不对可以继续修改。</span></li>
        <li><span class="rule-number">4</span><span>达到 3、7、12 个正确回答后，会解锁不同的结果与奖励。</span></li>
    ...
    if (panelDescription) {
      panelDescription.textContent = "输入答案并提交，系统会自动判断是否正确。";
    }

    const milestoneDescription = document.querySelector(".milestone-heading p");
    if (milestoneDescription) {
      milestoneDescription.textContent = "每一个正确答案都会计入通关进度。";
    }
  }
  ...
  if (resetButton) {
    resetButton.addEventListener("click", (event) => {
      ...
      const confirmed = window.confirm("确定重新开始吗？所有已输入答案和通关进度都会被清除。");
      if (!confirmed) return;
